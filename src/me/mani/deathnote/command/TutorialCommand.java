...
public class TutorialCommand extends DeathNoteCommand {
	...
	private PlayerRunnable[] actions = new PlayerRunnable[] {
		...
		(player) -> new Title("§cIdee §8[1/2]", "§7Idee ist es seine Gegner durch taktisches Blutabnehmen mit dem Messer", 5, 100, 5, false).send(player),
		(player) -> player.getInventory().setItem(4, ItemUtil.createItem(new ItemStack(Material.IRON_SWORD), "§7Messer")),
		(player) -> new Title("§cIdee §8[2/2]", "§7und Hilfe der 'Death Note' zu besiegen.", 5, 100, 5, false).send(player),
